...
def get_db_connection():
    if db_pool is None: raise Exception("DB Pool not initialized.")
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                # 1. Core Schema
                cur.execute("""CREATE TABLE IF NOT EXISTS departments (id SERIAL PRIMARY KEY, name TEXT UNIQUE NOT NULL, tier TEXT NOT NULL);""")
                cur.execute("""CREATE TABLE IF NOT EXISTS officers (id SERIAL PRIMARY KEY, username TEXT UNIQUE NOT NULL, passkey TEXT NOT NULL, full_name TEXT NOT NULL, department_id INTEGER REFERENCES departments(id), role TEXT NOT NULL);""")
                cur.execute("""CREATE TABLE IF NOT EXISTS tickets (id TEXT PRIMARY KEY, timestamp TIMESTAMPTZ DEFAULT NOW(), chat_id TEXT, user_name TEXT, raw_text TEXT, media_type TEXT, media_file_id TEXT, lat DOUBLE PRECISION, lon DOUBLE PRECISION, category TEXT, severity TEXT, summary TEXT, status TEXT DEFAULT '1. Pending Review', assigned_department_id INTEGER REFERENCES departments(id), assigned_officer_id INTEGER REFERENCES officers(id));""")
                cur.execute("""CREATE TABLE IF NOT EXISTS budget_ledgers (id SERIAL PRIMARY KEY, ticket_id TEXT REFERENCES tickets(id) ON DELETE CASCADE, budget_allocated NUMERIC DEFAULT 0, amount_spent NUMERIC DEFAULT 0, contractor_name TEXT, materials_used TEXT, resolution_media_id TEXT, audit_hash TEXT, updated_at TIMESTAMPTZ DEFAULT NOW());""")

                # 2. Seed Departments & Officers
                cur.execute("SELECT count(*) FROM departments")
                if cur.fetchone()[0] == 0:
                    cur.executemany("INSERT INTO departments (name, tier) VALUES (%s, %s) ON CONFLICT DO NOTHING", [('Govt of India', 'State'), ('Roads', 'Local'), ('Water', 'Local'), ('Electricity', 'Local'), ('Sanitation', 'Local'), ('Cyber Crime', 'Local'), ('Civil', 'Local')])
                
                cur.execute("SELECT count(*) FROM officers")
                if cur.fetchone()[0] == 0:
                    cur.executemany("INSERT INTO officers (username, passkey, full_name, department_id, role) VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING", [('collector', 'ias@india2026', 'District Collector', 1, 'admin'), ('pwd_roads', 'pwd@infra2026', 'Er. Rajesh Varma', 2, 'field'), ('water_board', 'jal@clean2026', 'Er. K. Ramesh', 3, 'field'), ('electricity', 'power@grid2026', 'Er. M. Praveen', 4, 'field'), ('sanitation', 'swm@clean2026', 'Dr. A. Rao', 5, 'field'), ('cyber_cop', 'cyber@cell2026', 'Inspector S. Reddy', 6, 'field')])

                # 3. Clean up any dummy data from old tests
                ...
                # 4. Safe Historical Data Injection (Your old tickets)
                cur.execute("SELECT count(*) FROM tickets WHERE id = 'GRV-0913124104'")
                if cur.fetchone()[0] == 0:
                    cur.execute("""
                        INSERT INTO tickets (id, chat_id, user_name, raw_text, lat, lon, category, severity, status, assigned_department_id) VALUES 
                        ('GRV-0913124104', '0', 'Citizen', 'Leaking drainage water', 17.350, 78.530, 'Sanitation', 'High', '2. Survey & Estimation', 5),
                        ('GRV-0913123149', '0', 'Citizen', 'Traffic signal broken', 17.340, 78.520, 'Roads', 'Medium', '6. Resolved (Social Audit)', 2),
                        ('GRV-0913122402', '0', 'Citizen', 'Dangerous pothole', 17.345, 78.525, 'Roads', 'Low', '1. Pending Review', 2)
                        ON CONFLICT DO NOTHING
                    """)
                    cur.execute("""
                        INSERT INTO budget_ledgers (ticket_id, budget_allocated, amount_spent) VALUES 
                        ('GRV-0913124104', 50000, 0), ('GRV-0913123149', 15000, 15000), ('GRV-0913122402', 0, 0)
                        ON CONFLICT DO NOTHING
                    """)
            conn.commit()
...
def analyze_and_route(text):
    ...
    prompt = f'Analyze this civic grievance: "{text}". Generate JSON: {{"category": "Roads|Water|Electricity|Sanitation|Cyber Crime|Civil", "severity": "High|Medium|Low", "summary": "1 concise sentence"}}'
    try:
        response = client.models.generate_content(model="gemini-2.5-flash", contents=prompt)
        return json.loads(response.text.replace("```json", "").replace("```", "").strip())
    except: return {"category": "Civil", "severity": "Medium", "summary": text[:80] if text else "Civic issue"}
...
def save_grievance(chat_id, user_name, raw_text, media_type, media_file_id, lat, lon):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            dept_name = cur.fetchone()[0] if cur.rowcount > 0 else "Pending Assignment"
            cur.execute("SELECT full_name FROM officers WHERE id = %s", (officer_id,))
            officer_name = cur.fetchone()[0] if cur.rowcount > 0 else "Awaiting Nodal Officer"
        conn.commit()
...
def get_all_complaints():
    ...
    except Exception as e: print(f"Fetch Error: {e}"); return []
